...
if st.session_state.cargo:
    ...
    for index, item in enumerate(st.session_state.cargo):

        cols = st.columns([4, 2, 2, 2, 2, 1])

        cols[0].write(item.description)
        cols[1].write(f"{item.length:.2f} × {item.width:.2f}")
        cols[2].write(f"{item.weight:.0f} kg")
        cols[3].write(f"× {item.quantity}")
