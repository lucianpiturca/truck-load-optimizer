...
if len(st.session_state.manifest) > 0:
    ...
    with c2:
        ...
        if total_gross > engine.max_total_weight:
            overw = total_gross - engine.max_total_weight
            st.error(
                f"OVERWEIGHT TOTAL! Gross: {total_gross:,} kg / "
                f"Max: 40,000 kg ({overw:,} kg OVER!)"
            )
        else:
            st.success(
                f"Total Gross Weight: {total_gross:,} kg / "
                f"Max: 40,000 kg (LEGAL)"
            )
        st.markdown("**Tractor Axles:**")
        if res['steer'] > engine.max_steer: st.error(f"Axle 1 (Steer): {res['steer']:,} kg")
        else: st.success(f"Axle 1 (Steer): {res['steer']:,} kg")
        if res['drive'] > engine.max_drive: st.error(f"Axle 2 (Drive): {res['drive']:,} kg")
        else: st.success(f"Axle 2 (Drive): {res['drive']:,} kg")
        st.markdown("**Trailer Axles:**")
        for idx, k in enumerate(['t1', 't2', 't3'], 3):
            v = res[k]
            if v > engine.max_axle_trailer: st.error(f"Axle {idx}: {v:,} kg")
            else: st.success(f"Axle {idx}: {v:,} kg")
    ...
    for idx, r in enumerate(layout, 1):
        ...
        if r['type'] == "DOUBLE":
            p_list = r['p']
            if len(p_list) == 2:
                # FIX: Added index markers to extract data from the row array
                p1_item = p_list[0]
                p2_item = p_list[1]
        ...
        grid_rows.append({"Position": f"{current_y} M", "Left Column": lc, "Right Column": rc, "Weight": r['wt']})
