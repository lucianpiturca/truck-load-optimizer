...
def generate_report(truck, result):
    ...
    if result.success:
        ...
        if not_loaded > 0:
            ...
            unloaded_description = format_unloaded_pallets(result.unloaded)
            if unloaded_description:
                lines.append(f"NOT LOADED: {unloaded_description}")
            else:
                lines.append(f"NOT LOADED: {not_loaded} {pallet_word}")

            lines.append(
                "Reason: no additional feasible position was generated by the selected loading pattern."
            )
...
def format_unloaded_pallets(unloaded):

    """Format pallet identities compactly, for example ``Apples #3-5``."""
    ...
    for description, unit_number in unloaded:
        grouped.setdefault(description, []).append(unit_number)
    ...
    for description, numbers in grouped.items():
        ...
        ranges.append(str(start) if start == end else f"{start}-{end}")
        groups.append(f"{description} #{', '.join(ranges)}")
